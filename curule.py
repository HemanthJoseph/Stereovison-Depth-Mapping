...
def main():
    ...
    img_0 = cv.imread("curule/im0.png") #reading left image
    resized_img_0 = rescaleFrame(img_0)
    gray_0 = cv.cvtColor(resized_img_0, cv.COLOR_BGR2GRAY)

    img_1 = cv.imread("curule/im1.png") #reading right image
    ...
    orb = cv.ORB_create(nfeatures = 450) #by default it takes 500 as max no. of features
    ...
    cv.imwrite("curule_image_outputs/keypoints_0.jpg", imgmatches_1)
    cv.waitKey(0)
    ...
    cv.imwrite("curule_image_outputs/keypoints_1.jpg", imgmatches_2)
    cv.waitKey(0)
    ...
    Fundamental_matrix = np.round(Fundamental_matrix , decimals =5)
    ...
    cv.imwrite("curule_image_outputs/epilines_0.jpg", img5)
    cv.waitKey(0)
    cv.imshow("epilines 1", img3)
    cv.imwrite("curule_image_outputs/epilines_1.jpg", img3)
    cv.waitKey(0)
    ...
    img_0_rectified = cv.warpPerspective(gray_0, H1, (w1, h1))
    img_1_rectified = cv.warpPerspective(gray_1, H2, (w2, h2))

    cv.imshow("img0 rectified", img_0_rectified)
    cv.imwrite("curule_image_outputs/rectified_0.jpg", img_0_rectified)
    cv.waitKey(0)
    cv.imshow("img1 rectified", img_1_rectified)
    cv.imwrite("curule_image_outputs/rectified_1.jpg", img_1_rectified)
    cv.waitKey(0)
    ...
    plt.imshow(disparity_map, cmap='hot', interpolation='bilinear')
    plt.savefig('curule_image_outputs/depth_image.png')
    plt.show()
